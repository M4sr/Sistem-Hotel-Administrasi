...
export default function EditFasilitasPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (!response.ok) {
        throw new Error("Gagal mengedit fasilitas")
      }
      router.push("/dashboard-admin-secure/fasilitas")
    } catch (error) {
      console.error("Error editing fasilitas:", error)
    }
  }
    ...
    <div className="container mx-auto py-6">
      <Card>
        <CardHeader>
          ...
          <CardDescription>Edit data fasilitas hotel</CardDescription>
        </CardHeader>
        <CardContent>
