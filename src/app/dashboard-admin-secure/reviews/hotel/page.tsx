...
export default function ReviewsHotelPage() {
  ...
  const fetchUlasan = async () => {
    setLoading(true)
    try {
      const response = await fetch(`/api/reviews/hotel?hotelId=${selectedHotel}`)
      const data = await response.json()
      setUlasan(data)
    } catch (error) {
  ...
  return (
    <div className="p-6">
      <h1 className="text-2xl font-bold mb-6">Ulasan per Hotel</h1>
        ...
        <Table>
          ...
          <TableBody>
            {loading ? (
              ...
              <TableRow>
                <TableCell colSpan={6} className="text-center">Tidak ada ulasan</TableCell>
              </TableRow>
            ) : (
